'''
Analisis_tanque-fuzzy v.1, 6 ago 2023
Tanque Fuzzy-Análisis
Autor: Damián Quijano A.
Versión con gráfica. Versión funcionable. 
...
nombreArchivoDatos='datos_1691379206.2286196.csv'
nombreArchvoIniciales='iniciales_1691379206.2286196.csv'


df=pd.read_csv(carpeta+nombreArchivoDatos)# ['iter', 'nivel_agua' , 'caudal_salida', 'caudal_neto'])
...
txt_duracion=df_iniciales.iloc[0][7]
'''
...
Ventana 2- cuadal de salida(resultado fuzzy): gráfico con los datos generales, gráfico con el tramo total, gráfico de oscilación 1,
gráfico de oscilación 2.
Se incluyen los datos: mínimo, máximo, intervalo, promedio.

Ventana 3- caudal neto: gráfico con los datos generales, gráfico con el tramo total, gráfico de oscilación 1, gráfico de oscilación 2.
Se incluyen los datos: mínimo, máximo, intervalo, promedio.
...
for i in df.itertuples():
    if i.nivel_agua>=setpoint and flag==1:# en nuestro caso 2.5 setpoint
        ...
        flag=0
    if grabar=="s":
        oscilacion1.append([i.iter,i.nivel_agua,i.caudal_salida, i.caudal_neto])
# A continuación se crea el dataframe del tramo de la oscilación      
df_oscilacion1 = pd.DataFrame( oscilacion1,columns=['iter','nivel','caudal_salida', 'caudal_neto'])
...
oscilacion2=[]
grabar="n"
iniciar_desde=150 # aquí se escribe un valor, por defecto es= iter_oscilacion1(calculado en el for de arriba al calcuar ka oscilación 1,que es igual al iter
...
for i in df.itertuples():
    if i.iter>=iniciar_desde:
        grabar="s"
    if grabar=="s":
        oscilacion2.append([i.iter,i.nivel_agua,i.caudal_salida, i.caudal_neto])
# A continuación se crea el dataframe del tramo de la oscilación      
df_oscilacion2 = pd.DataFrame( oscilacion2,columns=['iter','nivel','caudal_salida', 'caudal_neto'])
...
if len(df_oscilacion2)!=0:
    y_increment6 = (max_oscilacion2_caudal_salida-min_oscilacion2_caudal_salida) * 100 #OJO, en este caso si se pone 0.1 la gráfica demora mucho en aparecer
